...
import 'package:passenger/src/presentation/widget/track_driver_arrival/widget_tracking_map.dart';
          ...
          body: BlocConsumer<SingleRideCubit, SingleRideState>(
            listener: (context, state) {
              if (state is SingleRideSuccess) {
                BlocProvider.of<SingleDriverCubit>(context).monitorSingleDriver(context, state.data.driverReference ?? "");
                if (state.data.rideCurrentStatus == RideCurrentStatus.arrived) {
                  Navigator.of(context).pushReplacementNamed(AppRouter.rideNavigation, arguments: state.data.reference);
                }
              }
            },
            builder: (_, state) {
              if (state is SingleRideError) {
                ...
                return Stack(
                  children: [
                    Positioned(top: 0, right: 0, left: 0, bottom: 130, child: TrackDriverArrivalMap(ride)),
                    Positioned(
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        decoration:
                            BoxDecoration(borderRadius: BorderRadius.circular(16), color: theme.errorColor.withOpacity(.05)),
                        padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                        child: Text("on the way", style: TextStyles.body(context: context, color: theme.accentColor)),
                      ),
                    ),
