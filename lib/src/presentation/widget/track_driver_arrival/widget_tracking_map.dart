...
class _TrackDriverArrivalMapState extends State<TrackDriverArrivalMap> {
  ...
  void initState() {
    ...
    try {
      BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size.fromHeight(92)), 'images/$vehicleName-marker.png')
          .then((d) {
        setState(() {
          vehicleIcon = d;
        });
              ...
              setState(() {
                if (mapController != null) {
                  mapController!.animateCamera(CameraUpdate.newLatLngBounds(
                      cameraBounds(widget.ride, driver.location.latitude, driver.location.longitude), 42));
                }
              });
            }
          },
            ...
            if (state is SingleDriverSuccess) {
              ...
              return GoogleMap(
                initialCameraPosition:
                    CameraPosition(target: LatLng(driver.location.latitude, driver.location.latitude), zoom: 20),
                ...
                markers: [
                  Marker(
                    ...
                    icon: pickupMarkerIcon ?? BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueOrange),
                  ...
                  Marker(
                    ...
                    icon: vehicleIcon ?? BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueGreen),
                    anchor: Offset(0.5, 0.5),
                  ),
                ].toSet(),
                cameraTargetBounds:
                    CameraTargetBounds(cameraBounds(widget.ride, driver.location.latitude, driver.location.longitude)),
              );
            } else {
              return GoogleMap(
                initialCameraPosition:
  ...
  LatLngBounds cameraBounds(Ride ride, double lat, double lng) {
    if (ride.pickup.latitude > lat && ride.pickup.longitude > lng) {
      return LatLngBounds(southwest: LatLng(lat, lng), northeast: LatLng(ride.pickup.latitude, ride.pickup.longitude));
    } else if (ride.pickup.longitude > lng) {
      return LatLngBounds(southwest: LatLng(ride.pickup.latitude, lng), northeast: LatLng(lat, ride.pickup.longitude));
    } else if (ride.pickup.latitude > lat) {
      return LatLngBounds(southwest: LatLng(lat, ride.pickup.longitude), northeast: LatLng(ride.pickup.latitude, lng));
    } else {
      return LatLngBounds(southwest: LatLng(ride.pickup.latitude, ride.pickup.longitude), northeast: LatLng(lat, lng));
    }
  }
}
